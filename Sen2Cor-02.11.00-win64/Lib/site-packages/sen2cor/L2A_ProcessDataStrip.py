#!/usr/bin/env python
# -*- coding: iso-8859-15 -*-
# corrected wrong update of SIIMPC-1475 (line 338 ff.)
# includes patch for SIIMPC-1475 (line 332 ff.)
# second issue after problems occurred with abolute path of DEM directory (06/06/2019)
# includes patch for SIIMPC-1520 (lines 343, 365 ff.)
...
class L2A_ProcessDataStrip(): 
    ...
    def add_new_features(self):
        ...
        pi2a.UTC_DATE_TIME = strftime('%Y-%m-%dT%H:%M:%S', self.time.timetuple()) + 'Z'
        if self.processing_centre:    
        ...
        if self.config.productVersion > float32(14.6):
            pi2a.PRODUCT_DOI =  self.config.DOI
        ...
        if self.config.demDirectory == 'NONE':
            ...
            # SIIMPC-1475: check if DEM Directory exists:
            demDirectory = os.path.join(self.config.home, self.config.demDirectory)
            if os.path.isdir(demDirectory):
                item._setText(self.config.demReference)
            else:
                if self.config.operationMode == 'TOOLBOX':
                    os.makedirs(demDirectory)
                else:
                    self.logger.fatal('DEM directory does not exist')
                    item._setText('None')
                    return False

        self.time = datetime.utcnow()        
        ai2a = xp.getTree('General_Info', 'Archiving_Info')
        us = str(self.time.microsecond)[:6]
        if '000000' in us:
            ai2a.ARCHIVING_TIME = strftime('%Y-%m-%dT%H:%M:%S', self.time.timetuple()) + 'Z'
        else:
            ai2a.ARCHIVING_TIME = strftime('%Y-%m-%dT%H:%M:%S.', self.time.timetuple()) + us + 'Z'
    ...
    def generate(self):    
        ...
        try:
            ...
            try:
                ...
                sensingStart = datetime.strptime(sensingStart,'%Y-%m-%dT%H:%M:%S')
        ...
        if not found:
            self.logger.fatal('error in generating the L2A datastrip')
            return False
        else:
            self.logger.stream('L2A datastrip successfully generated')
            return found
