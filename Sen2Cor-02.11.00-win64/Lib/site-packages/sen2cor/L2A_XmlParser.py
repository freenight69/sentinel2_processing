# includes patch for SIIMPC-1541 (line 127)
...
class L2A_XmlParser(object):
    def __init__(self, config, product):
        ...
        if (product == 'GIPP'):
            ...
            self._xmlFn = config.L2A_QI_REPORT_XML
            self._scheme = config.qiReportScheme2a
        else:
    ...
    def validate(self):
        ...
        try:
            ...
            # workaround for SIIMPC-1541
            if err.message == "Element 'Image_Display_Order': This element is not expected. Expected is ( Spectral_Information_List ).":
                self._logger.info('L1C datastrip metadata is missing Spectral_Information_List section. Skipping validation.')
                ret = True
            else:
                ret = False
    ...
    def convert(self):
        ...
        outstr = outstr.replace('Radiometric_Offset_List', 'BOA_ADD_OFFSET_VALUES_LIST')
        outstr = outstr.replace('RADIO_ADD_OFFSET', 'BOA_ADD_OFFSET')
